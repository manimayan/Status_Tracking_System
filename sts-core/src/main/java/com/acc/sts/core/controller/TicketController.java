...
public class TicketController {
    ...
    public @ResponseBody ResponseEntity<List<UploadTicket>> listUploadTicket() {
        log.info("Retrieving list of Tickets which to be assigned");
        HttpStatus status = HttpStatus.NOT_FOUND;
        ...
        try {
          ...
          log.error("Error while Fetching list of Tickets which to be assigned", e);
          status = HttpStatus.INTERNAL_SERVER_ERROR;
        }
    ...
    public @ResponseBody ResponseEntity<Integer> ReassignUpdate(@PathVariable String employeeId,
        ...
        int response  = 0;
        try {
      	UploadTicket upTicket = new UploadTicket();;
  		upTicket = service.getTicketDetails(ticketId);
      	response = service.AssignUpdate(employeeId,ticketId,applicationName,upTicket);
      	status = HttpStatus.OK;
        } catch (Exception e) {
    ...
    public @ResponseBody ResponseEntity<List<Edit>> listticketdetails(@PathVariable("empId") String empId) {
        ...
        log.info("Retrieving list of details");
  ...
  
 {
        ...
        System.out.println("before try core controller ");
        HttpStatus status = HttpStatus.BAD_REQUEST;
        Ticket res = null;
        try {
        	System.out.println("in core controller before");
        	res = service.update1(ticketId,releaseTicket);
        	System.out.println("in core controller after");
            status = HttpStatus.CREATED;
        } catch (Exception e) {
            log.error("Error while saving updates ", e);
            status = HttpStatus.INTERNAL_SERVER_ERROR;
        }
    ...
    @GetMapping("/getname")
    public @ResponseBody ResponseEntity<List<Application>> getname() {
        log.info("Retrieving list of App");
        HttpStatus status = HttpStatus.NOT_FOUND;
        ...
        try {
            ...
            log.info("Error while Fetching list of App", e);
            status = HttpStatus.INTERNAL_SERVER_ERROR;
        }
        return new ResponseEntity<>(list, status);
    }
    ...
    public @ResponseBody ResponseEntity<List<ReassignTicket>> listReassignTicket() {
        log.info("Retrieving list of Employees(ticket_cont)");
        System.out.println("before core controller");
        HttpStatus status = HttpStatus.NOT_FOUND;
        List<ReassignTicket> listdetails = null;
        try {
        	listdetails = service.listReassignTicket();
            System.out.println("after core controller");
            status = HttpStatus.OK;
        } catch (Exception e) {
            log.info("Error while Fetching list of Employees", e);
            status = HttpStatus.INTERNAL_SERVER_ERROR;
    ...
    System.out.println("in web controller before ");
        ...
        System.out.println("core last"+list1);
        return new ResponseEntity<>(list1, status);
    }
    ...
    public @ResponseBody ResponseEntity<List<Application>> getAppname() {
        log.info("Retrieving list of Employees");
       System.out.println("in home core controller for application");
        HttpStatus status = HttpStatus.NOT_FOUND;
        List<Application> list = null;
        try {
    ...
    public @ResponseBody ResponseEntity<List<Employee>> getTestername() {
       log.info("Retrieving list of Employees");
      System.out.println("in home core controller for testername");
       HttpStatus status = HttpStatus.NOT_FOUND;
    ...
    public @ResponseBody ResponseEntity<List<Updatestatus>> listUpdatestatus() {
        ...
        try {
            ...
            System.out.println("in core controller try111"+list);
            status = HttpStatus.OK;
        } catch (Exception e) {
            log.info("Error while Fetching list of Employees", e);
            status = HttpStatus.INTERNAL_SERVER_ERROR;
    ...
    public @ResponseBody ResponseEntity<List<Closed>> listClosed(@PathVariable("empId") String empId) {
        log.info("Retrieving list of ticket c.controller");
System.out.println("In core controller");
        HttpStatus status = HttpStatus.NOT_FOUND;
      List<Closed> list2=null;
        try {
        	System.out.println("in core controller before");
            list2 = service.listClosedticket(empId);
            System.out.println("in core controller After");
            status = HttpStatus.OK;
        } /*catch (Exception e) {
            log.info("Error while Fetching list of tickets", e);
	...
	public @ResponseBody ResponseEntity<List<UploadTicket>> onchange(@RequestBody List<UploadTicket> tUpload) throws IOException {

		log.info("Save Employee details");
		System.out.println("core controller upload");
		HttpStatus status = HttpStatus.BAD_REQUEST;
